...
def draw_markers_on_image(img: Image.Image, gcps, pending, scale) -> Image.Image:
    """Dibuja los GCPs y el punto pendiente sobre la imagen de display."""
    arr = np.array(img.convert("RGB"))

    # GCPs confirmados
    for i, g in enumerate(gcps):
        cx, cy = int(g['px'] * scale), int(g['py'] * scale)
        cv2.circle(arr, (cx, cy), 10, (230, 57, 70), -1)
        cv2.circle(arr, (cx, cy), 10, (255, 255, 255), 2)
        cv2.putText(arr, str(i + 1), (cx + 12, cy + 5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (230, 57, 70), 2,
                    cv2.LINE_AA)

    # Punto pendiente (seleccionado pero sin mapa aún)
    if pending:
        cx, cy = int(pending[0] * scale), int(pending[1] * scale)
        cv2.circle(arr, (cx, cy), 12, (82, 183, 136), 2)
...
with col_img:
    ...
    # Escala para display (max 640 px de ancho)
    MAX_PX = 640
    scale = min(MAX_PX / img_w, MAX_PX / img_h, 1.0)
    disp = draw_markers_on_image(img, gcps, pending, scale)
    if scale < 1.0:
        disp = disp.resize(
    ...
    try:
        ...
        if coords is not None:
            real_x = coords['x'] / scale
            real_y = coords['y'] / scale
            # Solo actualizar si el clic es diferente al pendiente actual
